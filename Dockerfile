...
# Install postgres binaries to system for pgvector build
RUN cp -r /build/postgres/* /

# Build pgvector directly to system paths
WORKDIR /build
RUN git clone --branch ${PGVECTOR_VERSION} https://github.com/pgvector/pgvector.git pgvector
WORKDIR /build/pgvector
RUN set -ex; \
    make OPTFLAGS="" PG_CONFIG=/usr/lib/postgresql/$PG_MAJOR/bin/pg_config -j $(( $( (echo 2;grep -c ^processor /proc/cpuinfo||:;)|sort -n|tail -1) - 1 )); \
    make install PG_CONFIG=/usr/lib/postgresql/$PG_MAJOR/bin/pg_config; \
    # Copy pgvector files back to staging
    mkdir -p /build/postgres/usr/lib/postgresql/$PG_MAJOR/lib; \
    mkdir -p /build/postgres/usr/lib/postgresql/$PG_MAJOR/share/extension; \
